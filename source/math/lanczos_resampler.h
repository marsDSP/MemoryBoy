...
#ifndef MEMORYBOY_LANCZOS_RESAMPLER_H
#define MEMORYBOY_LANCZOS_RESAMPLER_H

#include "base_resampler.h"
...
namespace MarsDSP::Math
{
    ...
    public:
        ...
        LanczosResampler()
        {
            ...
            {
                double x0 = dx * static_cast<double>(t);
                for (size_t i = 0; i < filterWidth; ++i)
                {
                    double x = x0 + static_cast<double>(i) - A;
                    lanczosTable[t][i] = static_cast<float>(kernel(x));
                }
            }
            for (size_t t = 0; t < tableObs; ++t)
            {
        ...
        [[nodiscard]] float getResampleRatio() const noexcept override { return static_cast<float>(ratio); }
        ...
        size_t process(const float *input, float *output, size_t numSamples) noexcept override
        {
            ...
            return populateNext(output, static_cast<size_t>(static_cast<double>(numSamples) * ratio) + 1);
        }

    private:
        static constexpr size_t filterWidth = A * 2;
        static constexpr size_t tableObs = BUFFER_SIZE * 2;
        static constexpr double dx = 1.0 / static_cast<double>(tableObs);
        ...
        [[nodiscard]] double kernel(double x) const
        {
            if (fabs(x) < 1e-7)
                return 1;
        ...
        {
            state[wp] = f;
            state[wp + static_cast<int>(BUFFER_SIZE)] = f; // this way we can always wrap
            wp = (wp + 1) & (static_cast<int>(BUFFER_SIZE) - 1);
            phaseI += dPhaseI;
        }
        ...
        {
            double p0 = wp - xBack;
            auto idx0 = static_cast<int>(p0);
            idx0 = (idx0 + static_cast<int>(BUFFER_SIZE)) & (static_cast<int>(BUFFER_SIZE) - 1);
            if (idx0 <= static_cast<int>(A))
                idx0 += BUFFER_SIZE;
        ...
        [[nodiscard]] inline float readLin(double xBack) const
        {
            double p0 = wp - xBack;
            auto idx0 = static_cast<int>(p0);
            auto frac = static_cast<float>(p0 - idx0);
            idx0 = (idx0 + static_cast<int>(BUFFER_SIZE)) & (static_cast<int>(BUFFER_SIZE) - 1);
            if (idx0 <= static_cast<int>(A))
                idx0 += BUFFER_SIZE;
        ...
        {
            double p0 = wp - xBack;
            auto idx0 = static_cast<int>(floor(p0));
            double off0 = 1.0 - (p0 - idx0);

            idx0 = (idx0 + static_cast<int>(BUFFER_SIZE)) & (static_cast<int>(BUFFER_SIZE) - 1);
            idx0 += static_cast<int>(idx0 <= static_cast<int>(A)) * static_cast<int>(BUFFER_SIZE);

            double off0byto = off0 * tableObs;
            auto tidx = static_cast<int>(off0byto);
            double fidx = (off0byto - tidx);

            using SR = xsimd::batch<float>;
            auto rv = SR(0.0f);
            const auto fl = SR(static_cast<float>(fidx));
            for (size_t i = 0; i < filterWidth; i += SR::size)
            {
                ...
                auto dn = xsimd::load_unaligned(&state[idx0 - static_cast<int>(A) + static_cast<int>(i)]);
                rv += fn * dn;
            }
        ...
        void advanceReadPointer(size_t n) { phaseO += static_cast<double>(n) * dPhaseO; }
    ...
    float LanczosResampler<BUFFER_SIZE, A>::lanczosTable alignas (xsimd::default_arch::alignment())[
        LanczosResampler::tableObs + 1][LanczosResampler::filterWidth];

    template<size_t BUFFER_SIZE, size_t A>
    float LanczosResampler<BUFFER_SIZE, A>::lanczosTableDX alignas (xsimd::default_arch::alignment())[
        LanczosResampler::tableObs + 1][LanczosResampler::filterWidth];
